...
await oka.sendMessage(anu.id, { caption: `Haii @${num.split('@')[0]} 👋\nWelcome To ${metadata.subject}`, image: img, mentions: [num] })
...
await oka.sendMessage(anu.id, { caption: `Karena Untuk Setiap Ucapan Selamat Datang Akan Selalu Diakhiri Dengan Ucapan Selamat Tinggal 👋

Selamat Tinggal *@${num.split("@")[0]}* 👋`, image: baff, mentions: [num] })
} else if (anu.action == 'promote') {
oka.sendMessage(anu.id, { image: { url: ppuser }, mentions: [num], caption: `@${num.split('@')[0]} Ciee Jadi Admin Di Group ${metadata.subject}`  })
} else if (anu.action == 'demote') {
oka.sendMessage(anu.id, { image: { url: ppuser }, mentions: [num], caption: `*@${num.split('@')[0]}* Ciee Di Unadmin, Di Group ${metadata.subject}`})
  }
}
} catch (err) {
console.log(err)
      ...
      try {
        ...
        let ppuser2 = `https://telegra.ph/file/24fa902ead26340f3df2c.png`
          ...
          let lefts = `https://telegra.ph/file/e54e11c36ce7220047cbe.jpg`
            if ( anu.action === 'add' ) {
             await oka.sendMessage(anu.id, { image: {url: wlc}, caption: `Hay kak @${num.split("@")[0]}👋,\nSelamat Datang Di *${metadata.subject}*\n\nJangan Lupa Baca Desk\n*${metadata.desc}*`, mentions: [num]})
            } else if ( anu.action === 'remove' ) {
             await oka.sendMessage(anu.id, { image: {url: lefts}, caption: `Dadah @${num.split("@")[0]}👋,\nAgar Hubungan Baik Baik Saja, Adakah 10 Juta?`, mentions: [num]}) 
            } else if ( anu.action === 'promote' ) {
              oka.sendMessage(anu.id, { mentions: [num], text: `@${num.split("@")[0]} Selamat, Sekarang Anda adalah Admin Grup` })
            } else if ( anu.action === 'demote' ) {
...
start(`1`,`Connecting...`)
  ...
  oka.sendMessage('6285731947500' + "@s.whatsapp.net", { text: `*Ryo Versi 2*
